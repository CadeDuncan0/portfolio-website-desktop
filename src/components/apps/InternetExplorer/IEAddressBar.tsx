...
import { filterPages, IE_ADDRESSES, inputToRoute, pageUrl, resolvePage } from '@/config/ieRoutes'
...
export function IEAddressBar({ currentUrl, onOpentab, onNavigate, onRefresh }: IEAddressBarProps) {
  ...
  const results = dirty ? filterPages(query) : IE_ADDRESSES
...
/* Both glyphs copy the internet-explorer-{refresh,clear}.png reference bitmaps,
   ...
   Gradient ids go through useId because a literal would repeat once a second IE
   window opens, and every url(#…) then resolves to whichever gradient node is
